...
public class Main2Activity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        @SuppressWarnings("unused") public void onActivityResult(int requestCode, int resultCode, Intent data) {
            super.onActivityResult(requestCode, resultCode, data);

            if (requestCode == 1) {
                Toast.makeText(Main2Activity.this,"Uploading ...",Toast.LENGTH_LONG).show();
                Bitmap photo = (Bitmap) data.getExtras().get("data");
                Uri uri = getImageUri(this,photo);
                final String input = mLocationAddressTextView.getText().toString();
                uProgress.setVisibility(ProgressBar.VISIBLE);
                uploadbtn.setEnabled(false);
               final StorageReference filepath = mStorageRef.child("Photos").child(Calendar.getInstance().getTime().toString());
...
//                           }
//                       }, 500);
//                       mDatabaseRef.child("files").child(input).setValue(filepath.getDownloadUrl());
...
//                   }
                    ...
                    public void onComplete(@NonNull Task<Uri> task) {
                        if (task.isSuccessful()) {
                            ...
                            uProgress.setVisibility(ProgressBar.GONE);
                            uploadbtn.setEnabled(true);
                            Uri downloadUri = task.getResult();
                            Log.i("hello","hello");
                            Log.i("mytag",downloadUri.toString());
                            Map<String, Object> childUpdates = new HashMap<>();
                            Map<String, Object> childUpdates1 = new HashMap<>();
                            try {
                                childUpdates1.put("url",downloadUri.toString());
                                childUpdates1.put("loc",input);
                                String key = mDatabaseRef.child("files").push().getKey();
                                childUpdates.put("/files/"+key,childUpdates1);
